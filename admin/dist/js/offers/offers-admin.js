...
function loadOffers() {
  ...
  axios.get('../../../../../matthias-and-sea/admin/php/offers/load_offers.php')
    .then(response => {
...
function handleDeleteOffer(offerId, offerLanguage) {
  ...
  axios.post('../../../../../matthias-and-sea/admin/php/offers/delete_offer.php', formData)
    .then(response => {
...
function handleAddOfferForm() {
  ...
  form.addEventListener('submit', function (event) {
    ...
    axios.post('../../../../../matthias-and-sea/admin/php/offers/add_offer.php', formData, {
      headers: {
...
function handleFormSubmission() {
  ...
  form.addEventListener('submit', function (event) {
    ...
    // Update the language field to match the expected name in the PHP script
    formData.set('offerId', document.getElementById('offer-id').value);
    // Assuming you have these elements in your form with appropriate IDs
    const languageCodeElement = document.getElementById('language');
    const languageCode = languageCodeElement.value;

    // Map language codes to section names and language IDs
    ...
    if (!sectionMapping.hasOwnProperty(languageCode)) {
      console.error('Invalid language code:', languageCode);
    }

    const section = sectionMapping[languageCode];
    const languageId = languageIdMapping[languageCode];
    ...
    // Send a POST request to the PHP script using Axios
    axios.post('../../../../../matthias-and-sea/admin/php/offers/edit_offers.php', formData)
      .then(response => {
        if (response.status === 200) {
          // Handle a successful response, e.g., redirect to a success page
          // window.location.href = 'success_page.php' // Replace with the actual success page URL
