    ...
    axios.get('https://data.accentapi.com/feed/25386616.json?no_cache=20240324131712')
        .then(response => {
            ...
            for (const day in daysOfWeek) {
                const dutchDay = daysOfWeek[day];
                if (openHours[day]) {
                    tableContent += `<tr><td class="title">${dutchDay}:</td><td class="content">`;
                    if (openHours[day].length === 0) {
                        tableContent += "Gesloten";
                    } else {
                    ...
                    tableContent += `<tr><td class="title">${dutchDay}:</td><td class="content">Gesloten</td></tr>`;
                }
            }
